...
*   **墙壁行动（当剩余墙壁大于0时）：** 
	...
	**注：** 真实情况下墙壁位置的考量可以具备相当大的远见和策略深度，此处运用启发式思想选取关注位置附近的个别操作是为搜索算法可行性做出的牺牲。将自己的位置同样列为关注是增加策略深度的小尝试。
...
所有的代码均经过人工review和检视。
项目说明主要由本人编写，仅在游戏操作部分利用AI生成了部分内容。

**反思：** 
* 说实话我之前完全没有完整项目开发的经验，加上主题的选取主要来自于兴趣，对对弈AI实现的原理了解还相当有限，所以这次大作业的完成整体上对AI的依赖还是比较高的，从框架搭建到实现思路都或多或少接受了AI的指导。但所有内容（除了GUI部分）最终完成前都已经经过从头到尾的检视并完全理解，核心逻辑部分也都是本人理解后手动写的。
* AI目前的智能程度相当有限，设计上还有许多优化空间。
	* 有关**评估函数**的设计：还有剩余墙体数、所在位置等参数可以纳入考虑，应当通过AI对弈等方式得出最佳的参数设计。此外也可以考虑非线性模型。
	* 可以引入随机数增强AI不确定性和灵活性。
	* 此外，模型选择也有更优选择（如MCTS），Quoridor的复杂度实际上一定程度超出了Minimax算法的优势区间。但由于本人知识有限，本次大作业还是以通过实践掌握Minimax为主要目的，没有过多探索。